...
### Completato (modalità 100% locale — esclusi Supabase/Google Sheets)

- creazione progetto greenfield in `apps/scarichi-vini` con struttura modulare (routing `Home` / `Admin`)
...
- ottimizzazione asset logo:
  - `logo.webp` (primario) con fallback `logo.png`
...
### Non ancora avviato (rimane invariato)

- integrazione Supabase come database operativo centrale
